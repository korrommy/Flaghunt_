...
begin
  ...
  where public.challenges.id = p_challenge_id;
  ...
  is_correct := p_submitted_flag_hash is not null
    and p_submitted_flag_hash = challenge_row.flag_hash;

  insert into public.user_progress (user_id, challenge_id, attempts)
  values (current_user_id, p_challenge_id, 1)
  ...
  set is_solved = true,
      solved_at = now()
  where public.user_progress.user_id = current_user_id
    and public.user_progress.challenge_id = p_challenge_id;

  awarded_xp := challenge_row.xp_reward;
  update public.profiles
  set total_xp = public.profiles.total_xp + awarded_xp,
      level = floor(sqrt((public.profiles.total_xp + awarded_xp)::numeric / 100))::integer + 1
  where public.profiles.id = current_user_id
