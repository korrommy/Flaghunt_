...
export default function LoginPage() {
  ...
  async function handleSubmit(formData: FormData): Promise<void> {
    ...
    try { const { error } = await createBrowserClient().auth.signInWithPassword(parsed.data); if (error) { setErrorMessage("อีเมลหรือรหัสผ่านไม่ถูกต้อง"); return; } toast.success("เข้าสู่ระบบสำเร็จ"); router.replace(getNextPath(searchParams.get("next"))); router.refresh(); }
