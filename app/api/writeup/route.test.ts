...
import { POST } from "@/app/api/writeup/route";

const request = (body: string): Request => new Request("http://localhost/api/writeup", { method: "POST", headers: { "content-type": "application/json" }, body });

describe("POST /api/writeup", () => {
  it("rejects invalid challenge IDs", async () => {
    const response = await POST(request(JSON.stringify({ challenge_id: 0 })));
    expect(response.status).toBe(400);
    await expect(response.json()).resolves.toEqual({ success: false, error: "ข้อมูลไม่ถูกต้อง" });
  });
  it("requires verified claims", async () => {
