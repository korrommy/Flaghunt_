...
import { ChallengeFlagSubmission } from "@/components/game/ChallengeFlagSubmission";
...
export default async function ChallengePage({ params }: ChallengePageProps) {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <header>
        <p className="font-mono text-xs text-[#d4a84b]">CHALLENGE / {mission.difficulty.toUpperCase()}</p>
        <h1 className="mt-2 font-mono text-3xl text-gray-100">{mission.title}</h1>
      </header>
      <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        <aside className="space-y-4">
          <CommandPanel className="p-5">
            <h2 className="font-mono text-sm text-[#00ff88]">MISSION BRIEF</h2>
            <p className="mt-3 text-sm leading-6 text-gray-300">{mission.description}</p>
            <dl className="mt-5 grid grid-cols-2 gap-3 text-sm">
              <div><dt className="text-gray-500">รางวัล</dt><dd className="font-mono text-[#d4a84b]">{mission.xp_reward} XP</dd></div>
              <div><dt className="text-gray-500">สถานะ</dt><dd className="font-mono text-[#00ff88]">{isSolved ? "ผ่านแล้ว" : "กำลังทำ"}</dd></div>
            </dl>
          </CommandPanel>
          <HintPanel hintsUsed={hintsUsed} maxHints={mission.max_hints} />
          <WriteupPanel isSolved={isSolved} />
        </aside>
        <section className="space-y-4">
          <CommandPanel className="p-3"><TerminalEmulator script={getTerminalScript(mission.id)} /></CommandPanel>
          <CommandPanel className="p-5"><ChallengeFlagSubmission challengeId={mission.id} disabled={isSolved} /></CommandPanel>
        </section>
      </div>
    </div>
  );
